...
class QQEmailSlider(object):
    ...
    def _get_image(self, img_xpath):
        ...
        try:
            image_data = requests.get(img_url, cookies=cookies, verify=False, headers=headers)
        except Exception:
    ...
    def _find_similar_img(self, check_im):
        '''
        ...
        img_names = os.listdir(img_library_url)
        ...
        for name in img_names:
            ...
            for x_ in range(50):
                for y_ in range(50):
                    ...
                    r_, g_, b_ = check_im.getpixel((x_, y_))
                    # 像素差异在4以内，被认为是相同的
    ...
    def _drap_slider(self, slider_xpath, xoffset):
        ...
        total_x = 0     # 累积偏移量
        while True:
            xoffset = random.randint(self.min_xoffset, self.max_xoffset)      # 随机偏移像素
            yoffset = random.randint(-1, 1)
            total_x += xoffset
            if total_x > total_length:
                xoffset = xoffset - (total_x - total_length)
